package publish

import (
	"bytes"
	"encoding/base64"
	"encoding/json"
	"fmt"
	"net/http"
	"strings"
)

// giteaBackend publishes tools to a self-hosted Gitea instance.
type giteaBackend struct {
	cfg Config
}

func newGiteaBackend(cfg Config) (*giteaBackend, error) {
	if cfg.BaseURL == "" {
		cfg.BaseURL = "http://localhost:3000"
	}
	if cfg.Branch == "" {
		cfg.Branch = "main"
	}
	if cfg.BasePath == "" {
		cfg.BasePath = "registry/tools"
	}
	cfg.BaseURL = strings.TrimRight(cfg.BaseURL, "/")
	return &giteaBackend{cfg: cfg}, nil
}

func (g *giteaBackend) Name() string { return "gitea" }

func (g *giteaBackend) Upload(path string, content []byte) error {
	apiURL := fmt.Sprintf("%s/api/v1/repos/%s/%s/contents/%s",
		g.cfg.BaseURL, g.cfg.Owner, g.cfg.Repo, path)

	// Check if file exists to get SHA
	sha := ""
	req, _ := http.NewRequest("GET", apiURL, nil)
	req.Header.Set("Authorization", "token "+g.cfg.Token)
	if resp, err := http.DefaultClient.Do(req); err == nil && resp.StatusCode == 200 {
		var result map[string]interface{}
		json.NewDecoder(resp.Body).Decode(&result)
		resp.Body.Close()
		if s, ok := result["sha"].(string); ok {
			sha = s
		}
	} else if resp != nil {
		resp.Body.Close()
	}

	// Create/update file
	payload := map[string]interface{}{
		"content": base64.StdEncoding.EncodeToString(content),
		"message": fmt.Sprintf("feat(registry): publish %s", path),
		"branch":  g.cfg.Branch,
	}
	if sha != "" {
		payload["sha"] = sha
	}

	body, _ := json.Marshal(payload)
	req, _ = http.NewRequest("PUT", apiURL, bytes.NewReader(body))
	req.Header.Set("Authorization", "token "+g.cfg.Token)
	req.Header.Set("Content-Type", "application/json")

	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return fmt.Errorf("gitea: upload failed: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 && resp.StatusCode != 201 {
		return fmt.Errorf("gitea: upload returned HTTP %d", resp.StatusCode)
	}

	return nil
}
